...
"if not DEBUG" block at the bottom holds the production hardening, so a
deploy turns it on by setting DJANGO_DEBUG=False.
...
# Secrets and per-environment values come from the environment, never from
...
CORS_ALLOWED_ORIGINS = [
    origin.strip() for origin in os.environ.get("DJANGO_CORS_ALLOWED_ORIGINS", "").split(",")
    if origin.strip()
]
